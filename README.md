...
| **Twelve of fourteen backends executed and byte-identical against the certificate** | The differential harness COMPILES AND RUNS **Python, JavaScript, TypeScript, Go, Rust, C++, C#, Java, Kotlin, PHP, Lua, Ruby** over 16 recipes x 9 inputs -- three recipes taking TWO arguments, three inputs longer than the 512-element bound where the interpreter's clamp bites -- and diffs every line against `Recipe::apply`. All twelve match exactly. Lua and Ruby run as real Lua 5.4 and CRuby 3.4 compiled to WebAssembly: the implementations, not reimplementations. Swift and Haskell are emitted and **not counted** -- no toolchain installed yet, and writing a file is not evidence that it runs. Go proved that: emitted as `package kh`, it could never run at all while the harness printed a body-line count for it |
| **It does improve itself -- 18 to 25 against a bar that cannot move** | An evaluation set of 96 tasks drawn ONCE from a fixed seed over the base atoms, never regenerated, never filtered against anything learned, never admitted to a library. Between measurements the system trains on a DISJOINT stream and keeps what it certifies. **18 of 96 at the start, 25 after 768 training tasks -- while the empty-library control sits at exactly 18 at every stage.** The pipeline is deterministic and the flat control proves it, so the difference is attributable to the library and nothing else, with no interval needed |
