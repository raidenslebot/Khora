...
| **Five languages executed and byte-identical against the certificate** | 14 backends emit; the differential harness now COMPILES AND RUNS five of them -- **Python, JavaScript, Go, Rust, C++** -- over 13 recipes x 9 inputs, three of the inputs longer than the 512-element bound where the interpreter's clamp bites, and diffs every line against `Recipe::apply`. All five match exactly. Go had been emitted as `package kh`, so it could never be run at all and the harness reported a line count for it as though it had been |
