...
| **The residual ceiling, itemised** | Breaking the fixed 96-task bar down BY DEPTH: depth 2 solves 10 of 16 and learning adds nothing; depth 3-5 is where every gain lives (7->9, 1->3, 0->3); **depth 6-7 is zero before and after**. So it is two walls, not one. The shallow misses are not a search problem -- four of them are `altneg` and `idxmul`, index-aware functions the operation set cannot express at all. Adding `Op::Scan`, the exact inverse of the `Delta` already present and a name-for-name match with a missing atom, changed **nothing** |
