...
| **Six languages executed and byte-identical against the certificate** | 14 backends emit; the differential harness COMPILES AND RUNS six of them -- **Python, JavaScript, Go, Rust, C++, C#** -- over 16 recipes x 9 inputs, three of the recipes taking TWO arguments, three of the inputs longer than the 512-element bound where the interpreter's clamp bites, and diffs every line against `Recipe::apply`. All six match exactly. Go had been emitted as `package kh`, so it could never be run at all and the harness reported a line count for it as though it had been. PHP is emitted and NOT counted: there is no PHP here, and writing a file is not evidence that it runs |
