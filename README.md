...
| **Nine of fourteen backends executed and byte-identical against the certificate** | The differential harness COMPILES AND RUNS **Python, JavaScript, TypeScript, Go, Rust, C++, C#, Lua, Ruby** over 16 recipes x 9 inputs -- three recipes taking TWO arguments, three inputs longer than the 512-element bound where the interpreter's clamp bites -- and diffs every line against `Recipe::apply`. All nine match exactly. Lua and Ruby run as real Lua 5.4 and real CRuby 3.4 compiled to WebAssembly: the implementations, not reimplementations. Java, Kotlin, Swift, Haskell and PHP are emitted and **not counted** -- no toolchain here, and writing a file is not evidence that it runs. Go proved that: emitted as `package kh`, it could never run at all while the harness printed a body-line count for it |
