...
| **Raising reach is free; raising the curriculum is not** | A 2x2 on the same ascent. Adding `Op::ScanMax` and `Op::ScanMin` -- the running forms of `Max` and `Min`, the asymmetry `Delta` had before `Scan` -- **costs nothing at all: 232 vs 192 at tier 29, identical to baseline.** Adding the `cummax` ATOM costs **232 to 137** without those operations and **232 to 211** with them. So expressibility recovers 74 of the 95 lost, confirming the criterion, and the residual 21 is the intrinsic price of a harder curriculum. The operations are kept and the atom is not: the system can now write a running maximum in all 14 languages even though nothing in the curriculum asks for one |
