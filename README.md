...
| **10,000 lines of certified code in 22 seconds** | 450 lines/s on 24 threads, where every line comes from a program that passed every visible case AND every held-out case drawn longer than any it saw. Uncertified results contribute zero. Two counts were deliberately LOWERED for integrity: identity copies removed (3575 to 2474 lines, ~30%) and the prelude counted separately from bodies |
