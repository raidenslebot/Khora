...
| **Seven of fourteen backends executed and byte-identical against the certificate** | The differential harness COMPILES AND RUNS **Python, JavaScript, TypeScript, Go, Rust, C++, C#** over 16 recipes x 9 inputs -- three recipes taking TWO arguments, three inputs longer than the 512-element bound where the interpreter's clamp bites -- and diffs every line against `Recipe::apply`. All seven match exactly. Java, Kotlin, Swift, Haskell, Ruby, Lua and PHP are emitted and **not counted**: no toolchain here, and writing a file is not evidence that it runs. Go proved that -- it was emitted as `package kh`, could never run at all, and the harness printed a body-line count for it as though it had |
| **It does improve itself -- 18 to 26 against a bar that cannot move** | An evaluation set of 96 tasks drawn ONCE from a fixed seed over the base atoms, never regenerated, never filtered against anything learned, and never admitted to a library. Between measurements the system trains on a DISJOINT stream and keeps what it certifies. **18 of 96 at the start, 26 of 96 after 1,344 training tasks -- while the empty-library control sits at exactly 18 at every stage.** The pipeline is deterministic and the flat control proves it, so the difference is attributable to the library and nothing else. It also PLATEAUS: flat from stage 9, and raising the library budget 96 to 512 reaches 27 instead of 26. Five times the budget buys one task |
