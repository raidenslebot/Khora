...
| **10,000 lines of certified code in ~1.2 seconds, 85.0% certified** | The mandate is 10,000 lines in 30 s (333 lines/s). Every line comes from a program that passed every visible case AND every held-out case drawn longer than any it saw; uncertified results contribute zero. Measured: **8,190 to 9,139 lines/s across runs -- 24.6x to 27.4x the target** (the rate is noisy at about 6%, the certified count is not). At fixpoint **1,700 of 2,000 certified, 85.0%**, in 5 rounds. Peak pool **0.2 MB per shard**, which matters because RAM is the tight constraint here and not CPU |
