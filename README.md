...
| **It does improve itself -- 22 to 28 against a bar that cannot move** | An evaluation set of 96 tasks drawn ONCE from a fixed seed, never regenerated, never filtered against anything learned, never admitted to a library. Between measurements the system trains on a DISJOINT stream and keeps what it certifies. **22 of 96 at the start, 28 after 384 training tasks -- while the empty-library control sits at exactly 22 at every stage.** The pipeline is deterministic and the flat control proves it, so the difference is attributable to the library and nothing else, with no interval needed. It is a NET: 8 gained, 3 lost |
